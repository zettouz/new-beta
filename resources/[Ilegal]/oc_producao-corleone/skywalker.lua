...
oC = {}
Tunnel.bindInterface("oc_producao-corleone",oC)
-----------------------------------------------------------------------------------------------------------------------------------------
--[ ARRAY ]------------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
local corleone = {
...
--[ EVENTOS ]----------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("produzir-corleone")
AddEventHandler("produzir-corleone",function(item)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		for k,v in pairs(corleone) do
			if item == v.item then
				if item == "ak47" then
                    ...
                    if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("acessoalavagem") <= vRP.getInventoryMaxWeight(user_id) then
                        if vRP.getInventoryItemAmount(user_id,"pendrive") >= 10 then
                            if vRP.getInventoryItemAmount(user_id,"cartao") >= 10 then
                                if vRP.tryGetInventoryItem(user_id,"pendrive",1) and vRP.tryGetInventoryItem(user_id,"cartao",1) then
                                    TriggerClientEvent("fechar-nui-corleone",source)
                                    TriggerClientEvent("progress",source,10000,"Montando Acesso a Lavagem")
                                    TriggerClientEvent("corleone:posicao1",source)
                                    vRPclient._playAnim(source,false,{{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"}},true)
                                    SetTimeout(10000,function()
                                        vRPclient._stopAnim(source,false)
...
function oC.checkPermissao()
    local source = source
    local user_id = vRP.getUserId(source)
    if vRP.hasPermission(user_id,"corleone.permissao") then
        return true
    end
end