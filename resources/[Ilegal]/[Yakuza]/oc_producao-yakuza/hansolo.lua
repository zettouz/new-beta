...
oC = Tunnel.getInterface("oc_producao-yakuza")
-------------------------------------------------------------------------------------------------
--[ LOCAL ]--------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------
local prodMachine = {
	{ ['x'] = -924.28, ['y'] = -1467.46, ['z'] = 5.91 }
}
-------------------------------------------------------------------------------------------------
--[ MENU ]---------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------
...
RegisterNUICallback("ButtonClick",function(data,cb)
	if data == "produzir-ak47" then
		TriggerServerEvent("produzir-yakuza","ak47")
		timedown = 10

	elseif data == "produzir-ak74u" then
		TriggerServerEvent("produzir-yakuza","ak74u")
		timedown = 10

	elseif data == "produzir-uzi" then
		TriggerServerEvent("produzir-yakuza","uzi")
		timedown = 10

	elseif data == "produzir-pendrive" then
		TriggerServerEvent("produzir-yakuza","pendrive")
		timedown = 10

	elseif data == "produzir-acessoalavagem" then
		TriggerServerEvent("produzir-yakuza","acessoalavagem")
		timedown = 10
...
RegisterNetEvent("fechar-nui-yakuza")
AddEventHandler("fechar-nui-yakuza", function()
	ToggleActionMenu()
	onmenu = false

	local ped = PlayerPedId()
	local x,y,z = table.unpack(GetEntityCoords(ped))
	local bowz,cdz = GetGroundZFor_3dCoord(-924.28, -1467.46, 5.91)
	local distance = GetDistanceBetweenCoords(-924.28, -1467.46, cdz, x, y, z, true)

	if distance < 1.2 then
		TriggerEvent("yakuza:posicao1")	
	end
end)

RegisterNetEvent("yakuza:posicao1")
AddEventHandler("yakuza:posicao1", function()
	local ped = PlayerPedId()
	SetEntityHeading(ped, 110.12)
	SetEntityCoords(ped, -924.28, -1467.46, 5.91-0.95, false, false, false, false)
end)
-------------------------------------------------------------------------------------------------
--[ AÇÃO ]---------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------
