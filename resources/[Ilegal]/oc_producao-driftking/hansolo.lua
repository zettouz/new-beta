...
oC = Tunnel.getInterface("oc_producao-driftking")
-------------------------------------------------------------------------------------------------
--[ LOCAL ]--------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------
local prodMachine = {
	{ ['x'] = -1155.93, ['y'] = -2000.16, ['z'] = 13.19 }
}
-------------------------------------------------------------------------------------------------
--[ MENU ]---------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------
...
RegisterNUICallback("ButtonClick",function(data,cb)
	if data == "produzir-placa" then
		TriggerServerEvent("produzir-driftking","placa")
		timedown = 10

	elseif data == "produzir-raceticket" then
		TriggerServerEvent("produzir-driftking","raceticket")
		timedown = 10

	elseif data == "produzir-nitro" then
		TriggerServerEvent("produzir-driftking","nitro")
		timedown = 10
...
RegisterNetEvent("fechar-nui-driftking")
AddEventHandler("fechar-nui-driftking", function()
	ToggleActionMenu()
	onmenu = false

	local ped = PlayerPedId()
	local x,y,z = table.unpack(GetEntityCoords(ped))
	local bowz,cdz = GetGroundZFor_3dCoord(-1155.93, -2000.16, 13.19)
	local distance = GetDistanceBetweenCoords(-1155.93, -2000.16, cdz, x, y, z, true)

	if distance < 1.2 then
		TriggerEvent("driftking:posicao1")	
	end
end)

RegisterNetEvent("driftking:posicao1")
AddEventHandler("driftking:posicao1", function()
	local ped = PlayerPedId()
	SetEntityHeading(ped, 41.94)
	SetEntityCoords(ped, -1155.93, -2000.16, 13.19-0.95, false, false, false, false)
end)
-------------------------------------------------------------------------------------------------
--[ AÇÃO ]---------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------
