...
oC = Tunnel.getInterface("oc_producao-cosanostra")
-------------------------------------------------------------------------------------------------
--[ LOCAL ]--------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------
local prodMachine = {
	{ ['x'] = 3588.86, ['y'] = 3679.9, ['z'] = 27.63 }
}
-------------------------------------------------------------------------------------------------
--[ MENU ]---------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------
...
RegisterNUICallback("ButtonClick",function(data,cb)
	if data == "produzir-m-ak47" then
		TriggerServerEvent("produzir-cosanostra","m-ak47")
		timedown = 10

	elseif data == "produzir-m-advancedrifle" then
		TriggerServerEvent("produzir-cosanostra","m-advancedrifle")
		timedown = 10

	elseif data == "produzir-m-assaultsmg" then
		TriggerServerEvent("produzir-cosanostra","m-assaultsmg")
		timedown = 10

	elseif data == "produzir-m-microsmg" then
		TriggerServerEvent("produzir-cosanostra","m-microsmg")
		timedown = 10

	elseif data == "produzir-m-gusenberg" then
		TriggerServerEvent("produzir-cosanostra","m-gusenberg")
		timedown = 10

	elseif data == "produzir-m-weaponmg" then
		TriggerServerEvent("produzir-cosanostra","m-weaponmg")
		timedown = 10

	elseif data == "produzir-m-glock" then
		TriggerServerEvent("produzir-cosanostra","m-glock")
		timedown = 10

	elseif data == "produzir-m-snspistol" then
		TriggerServerEvent("produzir-cosanostra","m-snspistol")
		timedown = 10

	elseif data == "produzir-kevlar" then
		TriggerServerEvent("produzir-cosanostra","kevlar")
		timedown = 10

	elseif data == "produzir-colete" then
		TriggerServerEvent("produzir-cosanostra","colete")
		timedown = 10
...
AddEventHandler("fechar-nui-cosanostra", function()
	...
	local bowz,cdz = GetGroundZFor_3dCoord(3588.86, 3679.9, 27.63)
	local distance = GetDistanceBetweenCoords(3588.86, 3679.9, cdz, x, y, z, true)

	if distance < 1.2 then
		TriggerEvent("cosanostra:posicao1")	
	end
end)

RegisterNetEvent("cosanostra:posicao1")
AddEventHandler("cosanostra:posicao1", function()
	local ped = PlayerPedId()
	SetEntityHeading(ped, 173.29)
	SetEntityCoords(ped, 3588.86, 3679.9, 27.63-0.90,false,false,false,false)
end)
-------------------------------------------------------------------------------------------------
--[ AÇÃO ]---------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------
