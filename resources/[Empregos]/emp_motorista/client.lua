				...
				if distance <= 30.0 then
					...
					if distance <= 1.2 then
						...
						if IsControlJustPressed(1,38) then
							...
							TriggerEvent("Notify","bom","<b>Bus:</b> Olá, <b>Motorista</b>!",4000)
							TriggerEvent("Notify","bom","<b>Bus:</b> Os passageiros estão a sua espera!",5000)
						end
					end
				end
			end
			...
			if IsPedInAnyVehicle(ped) then
				...
				if IsEntityAVehicle(vehicle) then
					...
					if math.ceil(vehiclespeed) >= 71 and timers <= 0 and payment > 0 then
						timers = 5
						payment = payment 
						TriggerEvent("Notify","aviso","Limite de <b>70 mph</b> atingido e seu pagamento foi reduzido.",8000)
					end
				end
			end
		end
	...
	while true do
		...
		if emservico then
			if IsControlJustPressed(1,168) then
				emservico = false
				RemoveBlip(blip)
				TriggerEvent("Notify","aviso","<b>Bus:</b> Agradecemos pelo serviço prestado!",5000)
			end
		end
	end
end)
