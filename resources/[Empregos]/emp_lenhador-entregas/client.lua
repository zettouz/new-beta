...
--[ TRABALHAR ]--------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("testes",function(source,args)

	if not servico then
		if not varivaiel then
			segundos = 900
			servico = true
			varivaiel = true
			selecionado = 1
			...
			TriggerEvent("Notify","importante","Espere "..segundos.." segundos para realizar outra entrega")
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if servico then
			...
			if distance <= 30 and GetEntityModel(vehicle) == GetHashKey("pounder") then
