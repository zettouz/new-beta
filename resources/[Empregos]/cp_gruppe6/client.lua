...
job = Tunnel.getInterface("cp_gruppe6")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- INICIANDO TRABALHO
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('cp_gruppe6:permissao')
AddEventHandler('cp_gruppe6:permissao',function()
	if not emservico then
		local jobInfo = {
			['titulo'] = "TRANSPORTE DE VALORES",
			['descricao'] = "- Abasteça as ATM'S da Cidade</br> - Ganhe por Entrega</br> - Entregue os Pacotes de Dinheiro",
			['callBackEvent'] = "cp_gruppe6:checkIn"
		}
		checkIn.openJob(jobInfo)
	end
end)

RegisterNetEvent('cp_gruppe6:checkIn')
AddEventHandler('cp_gruppe6:checkIn',function()
	if checkIn.CheckTimer() == 0 then
		if checkIn.CheckJob() == false then
		    checkIn.SetJob(true)
	        emservico = true
	        parte = 0
	        TriggerEvent("Notify","importante","<b>Fred:</b> Certo, você está <b>contratado!</b> Pegue as chaves do <b>Carro Forte</b> com o <b>Davis</b> lá embaixo!")
	    else
	    	TriggerEvent("Notify","importante","<b>Fred:</b> Você está com a carteira assinada em um outro <b>Emprego</b>, aguarde!")
	    end
	else
		TriggerEvent("Notify","importante","<b>Fred:</b> Você acabou de sair de um <b>Serviço</b>, aguarde <b>"..checkIn.CheckTimer().." Segundos</b> para iniciar <b>Novamente</b>!")
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- GERANDO ENTREGA
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(ped), -41.78,-664.22,33.49,true) <= 30 then --SERVIÇO
		    -- DrawText3D(-4.77, -654.20, 33.45+0.47, "~w~Transporte de Valores", 1.0, 4)
		    -- DrawText3D(-4.77, -654.20, 33.45+0.35, "~b~Entregas de Pacotes de Dinheiro", 0.8, 1)
		 --   DrawMarker(39, -4.77, -654.20, 33.45-0.1, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 65, 105, 255, 50, 0, true, false, false)
		    if GetDistanceBetweenCoords(GetEntityCoords(ped), -41.78,-664.22,33.49,true) <= 1 then
                if IsControlJustPressed(0,38) then	
				    TriggerServerEvent('cp_gruppe6:permissao')
				end
			end
		end
		if emservico and parte == 0 then
			if GetDistanceBetweenCoords(GetEntityCoords(ped),-21.62,-711.56,32.34,true) <= 20 then --CARRO
		        --DrawMarker(21,0.41,-665.10,32.33-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,65,105,255,50,0,0,0,1)
		        if GetDistanceBetweenCoords(GetEntityCoords(ped),-21.62,-711.56,32.34,true) <= 1 then
                    if IsControlJustPressed(0,38) then	
                    	Fade(1200)
						TriggerEvent("Notify","importante","<b>Davis:</b> Estacione o <b>Carro Forte</b> do outro lado! Abra as portas e colete os <b>Pacotes de Dinheiro!</b>")
						-- TriggerEvent("Notify","importante","<b>Davis:</b> Abra as portas do <b>Carro Forte</b>, depois volte aqui e Mentalize <b>[E]</b> para coletar os <b>Pacotes de Dinheiro</b>!")
		                spawnStock()
                        parte = 1
				    end
			    end
		...
		if emservico and parte == 1 then
			...
			local distance = GetDistanceBetweenCoords(GetEntityCoords(ped),-34.71,-661.74,33.49,true)
			...
			if distance <= 50 and not pegando and portaaberta then
				--DrawMarker(21,0.37,-664.61,32.43-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,65,105,255,50,0,0,0,1)
				if distance < 1.5 and not IsPedInAnyVehicle(ped) then
                    if IsControlJustPressed(0,38) and not andamento then
                    	if quantidade < 15 then
                        	...
                        	TriggerEvent("Notify","aviso","Você colocou o máximo de <b>Pacotes</b> no <b>Carro Forte</b>!")
                        end
					end
				end
			end
			if distance2 <= 1.5 and pegando and not IsPedInAnyVehicle(ped) then
			    if model == 1747439474 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                    ...
                    if IsControlJustPressed(0,38) and andamento then
                        ...
                        if quantidade == 15 then
                        	TriggerEvent("Notify","importante","Você colocou <b>"..quantidade.."/15 Pacotes de Dinheiro</b> no <b>Carro Forte</b>, entre nele e vá abastecer os <b>Caixas Eletrônicos</b>!")
                        	vRP.playSound("5_SEC_WARNING","HUD_MINI_GAME_SOUNDSET")
                        	portaaberta = false
                        	parte = 2
                        else
                            vRP.playSound("5_Second_Timer","DLC_HEISTS_GENERAL_FRONTEND_SOUNDS")
                            TriggerEvent("Notify","importante","Você colocou <b>"..quantidade.."/15 Pacotes de Dinheiro</b> no <b>Carro Forte</b>!")
                        end
					end
				end
			end
				...
				if model == 1747439474 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                    if IsControlJustPressed(0,58) and not andamento then
                        if quantidade > 0 then
                            ...
                            TriggerEvent("Notify","importante","Você retirou <b>1x Pacote de Dinheiro</b> do <b>Carro Forte</b>, atualmente com <b>"..quantidade.."/15 Pacotes de Dinheiro</b>!")
                            vRP._CarregarObjeto("anim@heists@narcotics@trash","walk","prop_money_bag_01",50,57005,0.49,-0.18,0.00,25.0,270.0,180.0,2)
                            TirarCaixa()
                            andamento = true
                        else
                            parte = 1
                            vRP.playSound("5_SEC_WARNING","HUD_MINI_GAME_SOUNDSET")
                            TriggerEvent("Notify","aviso","Você entregou todas o seu <b>estoque</b>, volte para a central e pegue mais <b>Pacotes de Dinheiro</b>!")
                            quantidade = 0
						end
					end
				end
			...
			if distance <= 1.5 and not IsPedInAnyVehicle(ped) and retirando and entregando and portaaberta2 then
				if model == 1747439474 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                    ...
                    if IsControlJustPressed(0,58) and andamento then
				        ...
				        FreezeEntityPosition(ped,false)
				        ClearPedTasksImmediately(ped)
				        TriggerEvent("Notify","importante","Você colocou o <b>Pacote de Dinheiro</b> no <b>Carro Forte</b>, atualmente com <b>"..quantidade.."/15 Pacotes de Dinheiro</b>!")
				        vRP.playSound("5_Second_Timer","DLC_HEISTS_GENERAL_FRONTEND_SOUNDS")
					end
				end
			end
			for i = 1,20 do
			    ...
			    if distancecaixa <= 50 and entregando and retirando then
				    if distancecaixa < 1.3 and not IsPedInAnyVehicle(ped) then
					    DrawText3Ds(entregalocal[i].x,entregalocal[i].y,entregalocal[i].z+0.35,"~b~[H] ~w~Entregar o Pacote de Dinheiro")
					    TriggerEvent('cancelando',false)
                        if IsControlJustPressed(0,74) then
                        	TriggerServerEvent("cp_gruppe6:checkcaixa",entregalocal[i].id)
                        	if quantidade == 0 then
                        		parte = 1
                                vRP.playSound("5_SEC_WARNING","HUD_MINI_GAME_SOUNDSET")
                                TriggerEvent("Notify","aviso","Você entregou tudo, volte para a central e colete mais <b>Pacotes de Dinheiro</b>!")
                                quantidade = 0
                            end
                        end
	                end
	            elseif distancecaixa <= 50 and not entregando and not retirando then
				    ...
				    if distancecaixa < 1.5 and not IsPedInAnyVehicle(ped) then
				    	DrawMarker(21,entregalocal[i].x,entregalocal[i].y,entregalocal[i].z-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,65,105,255,50,0,0,0,1)
					    DrawText3Ds(entregalocal[i].x,entregalocal[i].y,entregalocal[i].z+0.35,"~w~Pegue o Pacote de Dinheiro no ~b~CARRO FORTE")
					end
                end
            end
		end
...
RegisterNetEvent('cp_gruppe6:startanim')
AddEventHandler('cp_gruppe6:startanim', function(id)
	Fade(1200)
	local pagamento = math.random(300,450)
    entregando = false
    retirando = false
    andamento = false
    vRP._DeletarObjeto()
    ClearPedTasksImmediately(PlayerPedId())
    TriggerServerEvent("cp_gruppe6:receber",pagamento)
    TriggerEvent("Notify","sucesso","<b>Pacote de Dinheiro</b> entregue, Você ganhou <b>$"..pagamento.."</b>, faltam entregar <b>"..quantidade.."/15 <b>Pacotes de Dinheiro</b>!")
			...
			if model == 1747439474 and GetVehicleNumberPlateText(veh) == vRP.getRegistrationNumber() then
                ...
                elseif traseira and not portaaberta then
	                ...
	                elseif parte == 2 and not entregando then
	            	    DrawText3Ds(coordsStock.x,coordsStock.y,coordsStock.z+0.95,"~b~[E] ~w~Fechar | ~b~[G] ~w~Pegar Pacote de Dinheiro")
	                elseif parte == 2 and entregando then
	            	    DrawText3Ds(coordsStock.x,coordsStock.y,coordsStock.z+0.95,"~b~[E] ~w~Fechar | ~b~[G] ~w~Guardar Pacote de Dinheiro")
	                end 
				    if IsControlJustPressed(0,38) then
	...
	if not nveh then
	 while not HasModelLoaded(mhash) do
	  RequestModel(mhash)
		...
		nveh = CreateVehicle(mhash,-19.91,-705.83,31.95+0.5,343.78,true,false)
