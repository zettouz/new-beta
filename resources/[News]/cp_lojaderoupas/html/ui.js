...
$(document).ready(function() {
    document.onkeydown = function(data) {
        if (data.keyCode == 27) {
            ...
            $('#total').html('0'); 
            change = {};
            $.post('http://cp_lojaderoupas/reset', JSON.stringify({}))
            $.post('http://ph-hud/fechar', JSON.stringify({ id: false }))
        }
    }

    $("#leftHeading").click(function() {
        $.post('http://cp_lojaderoupas/leftHeading', JSON.stringify({ value: 10 }));
    })

    $("#handsUp").click(function() {
        $.post('http://cp_lojaderoupas/handsUp', JSON.stringify({}));
    })

    $("#rightHeading").click(function() {
        $.post('http://cp_lojaderoupas/rightHeading', JSON.stringify({ value: 10 }));
    })

    $("#payament").click(function() {
        $(".loja-de-roupa").fadeOut()
        $.post('http://ph-hud/fechar', JSON.stringify({ id: false }));
        $.post('http://cp_lojaderoupas/payament', JSON.stringify({ price: $('#total').text() }));
        $('#total').html('0');
        change = {};
    })
...
function selectPart(element) {
    ...
    $.post('http://cp_lojaderoupas/changePart', JSON.stringify({ part: dataPart }))
}
...
function select(element) {
    ...
    $.post('http://cp_lojaderoupas/changeCustom', JSON.stringify({ type: dataPart, id: roupaID }));
}

$(".fa-angle-right").click(function() {
    $.post('http://cp_lojaderoupas/changeColor', JSON.stringify({ type: dataPart, id: roupaID, action: "mais" }));
})

$(".fa-angle-left").click(function() {
    $.post('http://cp_lojaderoupas/changeColor', JSON.stringify({ type: dataPart, id: roupaID, action: "menos" }));
})
