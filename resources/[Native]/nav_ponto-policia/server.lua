...
AddEventHandler("sair-servico-policia",function()
    ...
    if vRP.hasPermission(user_id,"paisana-bcso.permissao") then
        TriggerClientEvent("Notify",source,"negado","Você já está fora de serviço.")
    else
        vRP.addUserGroup(user_id,"paisana-bcso")
...
function logEntradaServico()
    ...
    PerformHttpRequest(policiaPonto, function(err, text, headers) end, 'POST', json.encode({embeds = {{ title = identity.name.." entrou em serviço.", description = "Registro de Ponto do Departamento de Polícia de Blaine. Registro de entrada em serviço.\n⠀", thumbnail = {url = "https://i.imgur.com/IP2d2mU.png"}, fields = {{ name = "**Nome do Agente:**", value = "` "..identity.name.." "..identity.firstname.." ` " },{ name = "**Nº Passaporte:**", value = "` "..user_id.." `\n⠀" }}, footer = { text = os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S"), icon_url = "https://i.imgur.com/IP2d2mU.png" },color = 15906321 }}}), { ['Content-Type'] = 'application/json' })
end
