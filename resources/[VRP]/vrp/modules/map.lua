...
function vRP.wantedReturn(user_id)
	if wanted[user_id] == 0 or not wanted[user_id] then
		return false
	else
		TriggerClientEvent("Notify",vRP.getUserSource(user_id),"importante","Aguarde "..vRP.getTimers(parseInt(wanted[user_id]))..".",5000)
		return true
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
...
function vRP.reposeReturn(user_id)
	if repose[user_id] == 0 or not repose[user_id] then
		return false
	else
		TriggerClientEvent("Notify",vRP.getUserSource(user_id),"importante","Aguarde "..vRP.getTimers(parseInt(repose[user_id]))..".",5000)
		return true
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
...
function vRP.wantedTimer(user_id,timer)
	if wanted[user_id] == 0 or not wanted[user_id] then
		wanted[user_id] = parseInt(timer)
	else
		wanted[user_id] = wanted[user_id] + parseInt(timer)
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- REPOSETIMER
...
function vRP.reposeTimer(user_id,timer)
	if repose[user_id] == 0 or not repose[user_id] then
		repose[user_id] = parseInt(timer) * 60
	else
		repose[user_id] = repose[user_id] + parseInt(timer) * 60
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- WANTED
...
RegisterCommand("procurado",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		if parseInt(wanted[user_id]) > 0 then
			TriggerClientEvent("Notify",source,"importante","Aguarde "..vRP.getTimers(parseInt(wanted[user_id]))..".",5000)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand("repouso",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		if parseInt(repose[user_id]) > 0 then
			TriggerClientEvent("Notify",source,"importante","Aguarde "..vRP.getTimers(parseInt(repose[user_id]))..".",5000)
		end
	end
end)