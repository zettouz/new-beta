...
function src.buyDealer(name)
	...
	if user_id then
		...
		elseif vRP.hasPermission(user_id,"supremo.permissao") then
			if parseInt(maxvehs[1].qtd) >= parseInt(maxgars[1].garagem) + 7 then
				TriggerClientEvent("Notify",source,"importante","Atingiu o número máximo de veículos em sua garagem.",8000)
				return
			end
		elseif vRP.hasPermission(user_id,"capeside.permissao") then
		...
		if vehicle[1] then
			...
			if vRP.tryFullPayment(user_id,vRP.vehiclePrice(name)*1) then
				...
				TriggerClientEvent("Notify",source,"sucesso","Você comprou um <b>"..vRP.vehicleName(name).."</b> por <b>$ "..vRP.format(parseInt(vRP.vehiclePrice(name)*1)).." dólares</b>.",10000)
...
--[[			if vRP.tryFullPayment(user_id,vRP.vehiclePrice(name)*0.90) then
				vRP.execute("creative/set_estoque",{ vehicle = name, quantidade = parseInt(rows2[1].quantidade) - 1 })
				vRP.execute("creative/add_vehicle",{ user_id = parseInt(user_id), vehicle = name, ipva = os.time() })
				TriggerClientEvent("Notify",source,"sucesso","Você comprou um <b>"..vRP.vehicleName(name).."</b> por <b>$ "..vRP.format(parseInt(vRP.vehiclePrice(name)*0.90)).." dólares</b>.",10000)
				src.updateVehicles(name,vRP.vehicleType(name))
				if vRP.vehicleType(name) == "carros" then
					TriggerClientEvent('dealership:Update',source,'updateCarros')
				elseif vRP.vehicleType(name) == "motos" then
...
function src.sellDealer(name)
	...
	if user_id then
		...
		if vehicle[1] then
			...
			vRP.setUData(user_id,"vRP:paypal",json.encode(parseInt(resultado + parseInt(vRP.vehiclePrice(name)*0.50))))
			--vRP.giveMoney(user_id,parseInt(vRP.vehiclePrice(name)*0.90))
			TriggerClientEvent("Notify",source,"sucesso","Você vendeu um <b>"..vRP.vehicleName(name).."</b> por <b>$"..vRP.format(parseInt(vRP.vehiclePrice(name)*0.50)).." dólares</b>.",10000)
			src.updateVehicles(name,vRP.vehicleType(name))
