RegisterNetEvent("b2k-character:startIntro")
...
function drawscaleform(scaleform)
	...
	PushScaleformMovieFunctionParameterString("~r~CAPESIDE")
	PushScaleformMovieFunctionParameterBool(true)
	PushScaleformMovieFunctionParameterString("~y~discord.gg/Kd8uuRs")
...
AddEventHandler("b2k-character:characterCreate", function()
	print("b2k-character:characterCreate")
	doStatus = 1
...
Citizen.CreateThread(function()
	while true do 
		...
		if continuousFadeOutNetwork then 
			...
			--print("continuousFadeOutNetwork Off")
			Citizen.Wait(1000)
		end
	end
end)
...
-- },
	...
	SetEntityCoordsNoOffset(PlayerPedId(), -252.11,-304.7,21.63,true,true,true)
	SetEntityHeading(PlayerPedId(),f(266.35))
...
RegisterNUICallback('cChangeHeading', function(data, cb)
    --print("Heading: " .. data.camRotation)
...
RegisterNUICallback('ChangeGender', function(data, cb)
    ...
    --TriggerServerEvent("b2k-character:setGender", data.gender)
