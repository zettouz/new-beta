...
$(document).ready(function() {
    document.onkeydown = function(data) {
        if (data.keyCode == 27) {
            ...
            $('#total').html('0'); 
            change = {};
            $.post('http://orp_clothes/reset', JSON.stringify({}))
            $.post('http://ph-hud/fechar', JSON.stringify({ id: false }))
        }
    }

    $("#leftHeading").click(function() {
        $.post('http://orp_clothes/leftHeading', JSON.stringify({ value: 10 }));
    })

    $("#handsUp").click(function() {
        $.post('http://orp_clothes/handsUp', JSON.stringify({}));
    })

    $("#rightHeading").click(function() {
        $.post('http://orp_clothes/rightHeading', JSON.stringify({ value: 10 }));
    })

    $("#payament").click(function() {
        $(".loja-de-roupa").fadeOut()
        $.post('http://ph-hud/fechar', JSON.stringify({ id: false }));
        $.post('http://orp_clothes/payament', JSON.stringify({ price: $('#total').text() }));
        $('#total').html('0');
        change = {};
    })
        ...
        if (item.openLojaRoupa) {
            ...
            for (var i = 0; i <= item.drawa; i++) {
                $("footer").append(`
                    ...
                    <div class="img-clothe" style="background-image: url('http://177.54.147.61/imgorigin/vrp_roupas/${item.category}/${item.sexo}/${item.prefix}(${i}).jpg')">  
                    <div class="overlay">
                                <span>${i}</span>
                            </div>
                        </div>
...
function selectPart(element) {
    ...
    $.post('http://orp_clothes/changePart', JSON.stringify({ part: dataPart }))
}
...
function select(element) {
    ...
    $.post('http://orp_clothes/changeCustom', JSON.stringify({ type: dataPart, id: roupaID }));
}

$(".fa-angle-right").click(function() {
    $.post('http://orp_clothes/changeColor', JSON.stringify({ type: dataPart, id: roupaID, action: "mais" }));
})

$(".fa-angle-left").click(function() {
    $.post('http://orp_clothes/changeColor', JSON.stringify({ type: dataPart, id: roupaID, action: "menos" }));
})
