...
local garages = { 
	...
	[6] = { locked = false, camera = { x = -770.37, y = -237.9, z = 37.08, h = 43.37 }, driveout = { x = -773.44, y = -234.78, z = 37.35, heading = 203.64 }, drivein = { x = -773.44, y = -234.78, z = 37.35, heading = 203.64 }, outside = { x = -773.44, y = -234.78, z = 37.35, heading = 203.64 }, inside = { x = -773.44, y = -234.78, z = 37.35, heading = 203.64 }},
	[7] = { locked = false, camera = { x = -766.31, y = -235.73, z = 37.08, h = 45.91 }, driveout = { x = -769.16, y = -232.67, z = 37.36, heading = 209.88 }, drivein = { x = -769.16, y = -232.67, z = 37.36, heading = 209.88 }, outside = { x = -769.16, y = -232.67, z = 37.36, heading = 209.88 }, inside = { x = -769.16, y = -232.67, z = 37.36, heading = 209.88 }}
}
										...
										if not LSC_Config.lock then
											if IsControlJustPressed(1,38) then
												if emP.checkPermission() then
													...
													TriggerEvent("Notify","aviso","Apenas <b>Mecânicos</b> podem modificar os veículos.",8000)
												end
											else
												sleep = 4
												drawTxt("PRESSIONE  ~r~E~w~  PARA ACESSAR A ~y~BENNYS",4,0.5,0.80,0.50,255,255,255,180)
											end
										else
											sleep = 4
											drawTxt("~y~BENNYS~w~ OCUPADA NO MOMENTO",4,0.5,0.80,0.50,255,255,255,180)
										end
									else
										if IsControlJustPressed(1,38) then
											if emP.checkPermission() then
...
function CanPurchase(price,canpurchase)
	if canpurchase then
		if LSCMenu.currentmenu == "main" then
			...
			TriggerEvent("Notify","bom","Modificação aplicada.",8000)
			--LSCMenu:showNotification("Modificação aplicada.")
		end
		return true
	else
	--	TriggerEvent("Notify","aviso","Dinheiro insuficiente.",8000)
