...
--[ WEBHOOK ]----------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
local logDropar = "SEUWEBHOOK"
local logEnviar = "SEUWEBHOOK"
local logEquipar = "SEUWEBHOOK"
-----------------------------------------------------------------------------------------------------------------------------------------
--[ VARIÁVEIS ]--------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
local actived = {}
		...
		if type == "usar" then
			...
			elseif itemName == "militec" then
				if not vRPclient.isInVehicle(source) then
					...
					if vehicle then
						if vRP.hasPermission(user_id,"devil.permissao") then
			...
			elseif itemName == "barracho" then
				if vRP.tryGetInventoryItem(user_id,"barracho",1) then
					...
					SetTimeout(30000,function()
						...
						TriggerClientEvent('cancelando',source,false)
						vRPclient._DeletarObjeto(source)
						TriggerClientEvent("Notify",source,"sucesso","Barracho utilizada com sucesso.",8000)
					end)
				end
			...
			elseif itemName == "buscopan" then
				...
				if vRP.tryGetInventoryItem(user_id,"buscopan",1) then
					...
					SetTimeout(10000,function()
						...
						TriggerClientEvent("remedios",source)
						vRPclient._DeletarObjeto(src)
						TriggerClientEvent("Notify",source,"sucesso","Você tomou <b>Buscopan</b>.")
					end)
				end
			end
