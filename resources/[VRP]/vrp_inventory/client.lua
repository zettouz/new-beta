...
AddEventHandler("remedios",function()
    ...
    if GetEntityHealth(ped) <= 239 then
        TriggerEvent("Notify","negado","<b>O remédio não fara efeito, pois você precisa de tratamento</b>.",8000)
    else
...
-----------------------------------------------------------------------------------------------------------------------------------------
local usandoRemedios = false
RegisterNetEvent("tandrilax")
AddEventHandler("tandrilax",function()
    local ped = PlayerPedId()
    local health = GetEntityHealth(ped)
    local armour = GetPedArmour(ped)
...
AddEventHandler("remedios2",function()
    ...
    if GetEntityHealth(ped) <= 149 then
        TriggerEvent("Notify","negado","<b>Ferimento extremamente grave, procure um Médico.</b>.",8000)
    else
        if usandoRemedios2 then
            return
        end

        usandoRemedios2 = true
