...
local radioData = {}
local callData = {}
local micClickVolumeSet = mumbleConfig.micClickVolume
...
RegisterCommand('micclick', function(source,args,rawCommand)
	if args[1] then
		micClickVolumeSet = tonumber(args[1])
	end
end)
	...
	if channel <= mumbleConfig.radioClickMaxChannel then
		if mumbleConfig.micClicks then
			if (snd and mumbleConfig.micClickOn) or (not snd and mumbleConfig.micClickOff) then
				SendNUIMessage({ sound = (snd and "audio_on" or "audio_off"), volume = micClickVolumeSet })
			end
		end
	end
end)
