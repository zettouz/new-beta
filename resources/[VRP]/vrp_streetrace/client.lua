	...
	while true do
		...
		if IsPedInAnyVehicle(ped) then
			if not inRace then
				...
				local distance = GetDistanceBetweenCoords(x,y,z,startX,startY,startZ,true)
				if distance <= 100 then
					timeDistance = 4
					DrawMarker(23,startX,startY,startZ-0.8,0,0,0,0,0,0,25.0,25.0,1.0,255,0,0,50,0,0,0,0)
					if distance <= 12.5 then
						dwText("PRESSIONE  ~b~E~w~  PARA INICIAR",4,0.5,0.93,0.38,255,255,255,50)
						if IsControlJustPressed(0,38) and timeSeconds <= 0 then
							timeSeconds = 2
				...
				local x,y,z = table.unpack(GetEntityCoords(ped))
				local distance = GetDistanceBetweenCoords(x,y,z,race[raceSelect][racePos][1],race[raceSelect][racePos][2],race[raceSelect][racePos][3],true)
				...
				if raceTime > 0 then
					timeDistance = 4
					dwText("RESTAM ~b~"..raceTime.." SEGUNDOS ~w~PARA ALCANÇAR O DESTINO FINAL DA CORRIDA",4,0.5,0.905,0.45,255,255,255,100)
					dwText("VENCA A CORRRIDA E PESQUISE SEUS PROPRIOS REGISTROS ANTES QUE O TEMPO ACABE",4,0.5,0.93,0.38,255,255,255,50)
				end
			end
		end
	...
	while true do
		if inRace and raceTime > 0 then
			...
			if raceTime <= 0 or not IsPedInAnyVehicle(PlayerPedId()) then
				...
				vSERVER.stopRace(GetVehicleNumberPlateText(GetPlayersLastVehicle()))
