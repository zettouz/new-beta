...
--[ WEBHOOKS ]---------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookDepositar = "SEUWEBHOOK"
local webhookSacar = "SEUWEBHOOK"
local webhookPTrans = "SEUWEBHOOK"
local webhookPSacar = "SEUWEBHOOK"
local webhookRCaixaEletronico = "SEUWEBHOOK"
...
function banK.checkRobbery(id,x,y,z,head)
	...
	if user_id then
		...
		elseif (os.time()-timers) <= 800 then
			TriggerClientEvent("Notify",source,"aviso","Os caixas estão vazios, aguarde <b>"..vRP.format(parseInt((800-(os.time()-timers)))).." segundos</b> até que os civis depositem dinheiro.",8000)
		else
			andamento = true
			timers = os.time()
			dinheirosujo = {}
...
function banK.checkPermission()
	local source = source
	local user_id = vRP.getUserId(source)
	return not (vRP.hasPermission(user_id,"dpla.permissao") or vRP.hasPermission(user_id,"dmla.permissao") or vRP.hasPermission(user_id,"paisana-dpla.permissao"))
end
