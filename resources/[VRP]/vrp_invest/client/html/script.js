            ...
            for (var e in array) {
                ...
                $('#companies tbody').append(`
                    <tr data-label='${obj.label}'>
                        <th>${obj.name}</th>
                        <th><i class='fas ${icon}'></i> ${obj.stock.toFixed(2)}</th>
                    </tr>`)
            }
...
$('form .btn').click(function (e) {
    ...
    if(!inputValue || /^\d+$/.test(inputValue.value)) {
        ...
        if (activeMenu == "sell") {
            $.post('http://vrp_invest/sellInvestment', JSON.stringify({job: label}))
        } else if(activeMenu == "buy") {
            $.post('http://vrp_invest/buyInvestment', JSON.stringify({job: label, amount: inputValue, boughtRate: rate}))
        }
        $('#amount').val('');
        mainPage()
    }
...
$('#new_bank').click(function() {
    $.post('http://vrp_invest/newBanking', "{}")
})
...
$('#buy').click(function() {
    $('#general').hide();
    $('#buyUI').show();
    $('#close').html("Voltar <i class='fas fa-sign-out-alt'></i>");
    $.post('http://vrp_invest/list', "{}")
    inMenu = true
    activeMenu = "buy"
})
...
function mainPage() {
    $('#close').html("Fechar <i class='fas fa-sign-out-alt'></i>");
    $('#buyUI, #allUI, #sellUI').hide();
    $('#general').show();
    inMenu = false
    $.post('http://vrp_invest/balance', "{}")
}

function close() {
    $('#general, #waiting, #sellUI, #allUI, #buyUI, #topbar').hide();
    $('body').removeClass("active");
    $.post('http://vrp_invest/close', "{}");
}