TokoVoipConfig = {
	...
	plugin_data = {
		...
		TSChannel = "[JOGANDO]",
		TSPassword = "senha", -- TeamSpeak channel password (can be empty)
		...
		TSServer = "NOME.NOME.com.br", -- TeamSpeak server address to be displayed on blocking screen
		TSChannelSupport = "https://discord.gg/Kd8uuRs", -- TeamSpeak support channel name displayed on blocking screen
		TSDownload = "https://discord.gg/Kd8uuRs", -- Download link displayed on blocking screen
		...
		localName = "NOME", -- If set, this name will be used as the user's teamspeak display name
...
AddEventHandler("onClientResourceStart", function(resource)
	if (resource == GetCurrentResourceName()) then	--	Initialize the script when this resource is started
		Citizen.CreateThread(function()
			TokoVoipConfig.plugin_data.localName = escape("[NOME]"); -- Set the local name
		end);
